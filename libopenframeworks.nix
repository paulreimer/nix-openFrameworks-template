...
pkgs.stdenv.mkDerivation rec {
  ...
  buildPhase = ''
    ninja --verbose -C out openFrameworks
  '';

  installPhase = ''
    mkdir -p $out/include/
    rsync -a --include="*/" --include="*.h" --exclude="*" openFrameworks/libs/openFrameworks/ $out/include/

    mkdir -p $out/lib/
    mv ./out/obj/*.a $out/lib/
  '';
